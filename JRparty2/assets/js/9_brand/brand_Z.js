...
require(['jquery', 'jquery.cookie', 'bootstrap', 'bootstrap-table', 'bootstrap-select',
    'bootstrap-datetimepicker', 'bootstrap-datetimepicker.zh-CN', "fileinput", "fileinput_locale_zh"], function ($) {
        ...
        $remove = $('#table-remove'),
        selections = [];
        $table.attr("data-url", "/JRPartyService/Brand.svc/getBrandList?districtID=" + $.cookie('JTZH_districtID') + "&");
        function initTable() {
        ...
        function operateFormatter(value, row, index) {
            return [
                        ...
                        // '</a> ',
                        // '<a class="grade am-btn"  style="color:#337ab7" href="javascript:void(0)"  title="评级" >',
                        //'<i class="glyphicon glyphicon-edit"></i>评级',
                        //'</a>',
        ...
        window.operateEvents = {
            ...
            'click .check': function (e, value, row, index) {
                window.open('brand_detail.html?id=' + row.id)
                //$.ajax({
                //    url: svc_brand + "/getsingleBrand?",    //后台webservice里的方法名称 
                //    data: {
                //        id: row.id
                //    },
                //    type: "get",
                ...
                //            str = str + '<a style="line-height:2.5" href="' + svc_file + data.data.imageURL[i].Url + '"target="_blank"><img src="../assets/img/file.png" />' + data.data.imageURL[i].Url + '</a><br>'
                //        }
                //        $('#common-alert2 .modal-title').html('');
                //        $('#common-alert2 .modal-title').html('文件');
                //        $('#common-alert2 .modal-body').html('');
                //        $("#common-alert2 .modal-body").html('<div class="form-group">' +
                //                                               '<label for="brand">品牌内容：</label><br>' + row.description +
                //                                               '</div><div class="form-group">' +
                //                                               '<label for="exampleInputName2">文件：</label><br>' + str + '</div>');
                //        $('#common-alert2').modal();
                //    }
                //})
            ...
            'click .grade': function (e, value, row, index) {
                $('#table_grade_modal .modal-title').html('');
                $('table_grade_modal .modal-title').html('评级');
                $('#table_grade_modal .modal-body').html('');
                $("#table_grade_modal .modal-body").html('<div class="form-group">' +
                                                       '<label for="brand">品牌内容：</label>' +
                                                       '<select id="table_grade_modal-rate" class="form-control selectpicker" title="品牌内容"> ' +
                ...
                $('#table_grade_modal-rate').selectpicker('val', (row.rate));
                $('#table_grade_modal').modal();

                $("#table_grade_modal .btn-success").unbind("click");
                $('#table_grade_modal').find('.btn-success').click(function () {
                    $.ajax({
                        url: svc_brand + "/rateBrand",
                        type: "GET",
                        data: {
                            id: row.id,
                            rate: $('#table_grade_modal-rate').val(),
                        },
                        success: function (data) {
                            $('#table_grade_modal').modal('hide');
                ...
                $.ajax({
                    ...
                    success: function (data1) {
                        ...
                        for (var i in data1.rows) {
                            organ = organ + '<option value="' + data1.rows[i].id + '">' + data1.rows[i].name + '</option>'
                        }
                        $.ajax({
                            type: "GET",
                            url: svc_brand + "/getsingleBrand",
                            data: {
                                id: row.id
                            },
                            success: function (data) {
                                ...
                                $('#table_edit_modal .modal-title').html('编辑品牌');
                                var str = '<form id="editform">' +
                                            '<div class="form-group">' + 
                                                  '<label for="table_edit_modal-districtID">所属组织</label>' +
                                                '<select id="table_edit_modal-districtID"  name="districtID" class="form-control selectpicker" title="部门"> ' + organ + '</select>' +
                                                 '</div>' +
                                                  '<div class="form-group">' +
                                                    '<label for="table_edit_modal-title">名称</label>' +
                                                    '<input type="text" class="form-control"name="title" id="table_edit_modal-title" >' + 
                                ...
                                $('#table_edit_modal-districtID').selectpicker('val', (data.data.districtID));
                                $("#table_edit_modal-id").val(row.id);
                                $("#table_edit_modal-title").val(row.title);
        ...
        $('#table-add').click(function () {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('#table_add_modal .modal-title').html('新增品牌');
                    var str = '<form id="form"><div class="form-group">' +
                              ...
                              '</div>' +
                           '<div class="form-group">' +
                            '<label for="table_add_modal-title">名称</label>' +
                          '<input type="text" class="form-control"name="title" id="table_add_modal-title" >' +
                          '</div>' +
                          //  '<div class="form-group">' +
...
function doUpload() {
    ...
    $.ajax({
        url: svc_uoload + '/BrandUpload.ashx?',
        type: 'POST',
        data: formData,
        async: false,
        cache: false,
...
function delet(id) {
    ...
    $('#' + id).remove();
    $.ajax({
        url: svc_position + '/deleteActivityFile',
        type: 'GET',
        data: {
            id: id
        },
