...
require(['jquery', 'jquery.cookie', 'bootstrap', 'bootstrap-table', 'bootstrap-select',
    'bootstrap-datetimepicker', 'bootstrap-datetimepicker.zh-CN', "fileinput", "fileinput_locale_zh"], function ($) {
        ...
        $remove = $('#table-remove'),
        selections = [];
        $table.attr("data-url", "/JRPartyService/Team.svc/getReserveCadreList?districtID=" + $.cookie('JTZH_districtID') + "&");
        function initTable() {
            $table.bootstrapTable({
                ...
                onLoadSuccess: function () {
                    ...
                    //     $table = $("#table");
                    //     var obj = getObjFromTable2($table, "town");
                    //
                    //     for (var item in obj) {
                    //         $("#table").bootstrapTable('mergeCells', {
                    //             index: obj[item].index,
        ...
        window.operateEvents = {
            ...
            'click .edit': function (e, value, row, index) {
                $.ajax({
                    type: "GET",
                    url: svc_team + "/getReserveCadreDetail",
                    data: {
                        id: row.id
                    },
                    success: function (data) { 
                      ...
                      '</div>' +
                       '<div class="form-group">' +
                      '<label for="table_edit_modal-TrainingTitle">培训名称</label>' +
                      ...
                      '</div>' +
                       '<div class="form-group">' +
                      '<label for="table_edit_modal-JionTime">加入时间</label>' +
            ...
            'click .remove': function (e, value, row, index) {
                ...
                $(".confirm").click(function () {
                    ...
                    $.ajax({
                        type: "GET",
                        url: svc_team + "/deleteReserveCadre",//http://172.16.0.221:8006/JRPartyService/Party.svc/deletePlan?id={id}
                        data: {
                            id: row.id
                        },
...
function doUpload() {
    ...
    $.ajax({
        url: svc_uoload + '/ReserveCadreUpload.ashx?',
        type: 'POST',
        data: formData,
        async: false,
        cache: false,
...
function doEdit() {
    ...
    $.ajax({
        url: svc_uoload + '/editReserveCadre.ashx?',
        type: 'POST',
        data: formData,
        async: false,
        cache: false,
