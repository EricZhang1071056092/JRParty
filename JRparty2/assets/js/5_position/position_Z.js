...
require(['jquery', 'jquery.cookie', 'bootstrap', 'bootstrap-table', 'bootstrap-select',
    'bootstrap-datetimepicker', 'bootstrap-datetimepicker.zh-CN', "fileinput", "fileinput_locale_zh"], function ($) {
        ...
        $remove = $('#table-remove'),
        selections = [];
        $table.attr("data-url", "/JRPartyService/Position.svc/getPositionList?districtID=" + $.cookie('JTZH_districtID') + "&");
        function initTable() {
                    ...
                    success: function (data1) {
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                ...
                                for (var i in data.data.imageURL) {
                                    console.log(data.data.imageURL[i].id);
                                    strfile = strfile + '<span  id="' + data.data.imageURL[i].id + '"><a style="line-height:2.5" href="' + svc_file + data.data.imageURL[i].Url + '"target="_bank"><img src="../assets/img/file.png" />' + data.data.imageURL[i].Url + '</a>&nbsp;&nbsp;&nbsp;<a href="#"style="line-height:2.5"><img src="../assets/img/1_index/remove.png"onclick="delet(\'' + data.data.imageURL[i].id + '\')"width="20" style="margin-top:8px" /></a></span>'
                                }
                                ...
                                var str = '<form id="editform">' + 
                                     ...
                                     '<select name="type" id="table_edit_modal-type"  class="form-control selectpicker" title="名称"> ' +
                             ...
                             '</select>' +
                                      ...
                                      ' </div>' +
                                      '<div class="form-group">' +
                                      '<label for="exampleInputName2">已上传：</label>' + strfile
                                     ' </div>' + 
                                '</div>' +
                                '</form>'
                                ...
                                $('#table_edit_modal-districtID').selectpicker('val', (row.districtID));
