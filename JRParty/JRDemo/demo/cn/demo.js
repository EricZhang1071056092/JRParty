...
var StudyContent = getParam('StudyContent');
var StudyId = getParam('StudyId');
...
// console.log(PartyBranch);
// console.log(StudyContent);
// console.log(number);
/**
 document.getElementById('title-span').innerText=PartyBranch;
 document.getElementById('maqueue-span1').innerText=content+'正在进行中' ;
 document.getElementById('maqueue-span2').innerText=content + '正在进行中' ;;
 document.getElementById('maqueue-span3').innerText=content +'正在进行中' ; ;*/
...
$(document).ready(function(){
    $('#StudyTitle').text(getParam('StudyTitle'));
    $('#StudyType').text(getParam('StudyType'));
    $('#StudyContent').text(getParam('StudyContent'));
})
 ...
 $('#testDiv').html(number);
    $.ajax({
        ...
        success: function (data) {
            //console.log(data);
			// tempNum++;
			// $('#testDiv').html(tempNum);
            if((data.data!="") && (data.data!=null)){
				
                var str="http://172.16.0.221:8006/JRPartyService/picture/"+data.data;
                $('#screenshot').css('backgroundImage',"url('"+str+"')");
                $('#nowadayPictrue').css('backgroundImage',"url('"+str+"')");
				//$('#testDiv').html(str);
            }
        }
    })
...
function keyDown(evt) {
    ...
    switch (keyCode) {
        ...
        case KEY.DOWN:
            ...
            break;
        case KEY.UP:
			//iAlert(navigator.appVersion);
            break;
        case KEY.LEFT:
            // window.location.href='./test/temp.html';
            break;
        case KEY.RIGHT:
            break;
        case KEY.ENTER:
            ...
            switch (FOCUS) {
                case "BUT_0":
					timeEnd=setTimeout(clearTimer,60*60*3*1000);
                    pictureTime=parseInt(time);
                    $('#StartRealPlayFocus').attr('src','img/end.png');
                    setXSJYLTime();
                    setIntervalFun = setInterval(setXSJYLTime, 60000 * pictureTime);
                    //console.log($('#StartRealPlayFocus'));
                    //$('#testDiv').text(keyCode);
                    FOCUS = "BUT_1";
                    break;
                case "BUT_00":
                ...
                case "BUT_1":
                    ...
                    $('#StartRealPlayFocus').attr('src','img/kaishihuiyi.png');
                    FOCUS = "BUT_0";
                    break;
            }
            break
...
$.ajax({
	...
	success: function (data) {
		...
		if (data.success == true) {
			...
			//    console.log(IP, port, ChannelID, userName, password);
			time=data.data.time;
			//console.log(time);
			$('#loginip').val(IP);
			$('#port').val(port);
			...
			$('#deviceport').val(ChannelID);
			clickLogin();
			//  alert(IP);
			$('#StartRealPlayFocus').click()
			play(data.data.IP);
		} else {
...
function play(src)
{
    Utility.setSpeakerOutputMode(1);
    //var deliver = "rtsp://admin:admin_96296@172.16.1.130:554/h264/ch10/main/av_stream";
    //var deliver="http://172.16.0.221:8006/video/01.m3u8"
    //var deliver = "http://172.16.0.218/hls/evmtv_172.16.1.130-554-av_stream/evmtv_172.16.1.130-554-av_stream-.m3u8";

    //var deliver = "http://172.16.0.221:8006/kjr/20160309t083928-02-0.ts";
    //var deliver = "http://172.16.0.221:8006/kjr/aaa.mp4";
    //var deliver = "http://172.21.2.90:8112/hls/01000000000000000000000000000397";
    //var deliver = "http://192.168.2.115/test/2m.ts.m3u8";
    //var deliver = "http://192.168.2.89/huaian/videomonitor/sudu7.mp4";
    //var deliver = "http://172.16.0.221:8006/kjr/output.mp4";
    //var deliver = "deliver://275000000.6875.64.799"; // for huaian
    //var deliver = "http://172.16.0.31:6713/mag/hls/0efc00cd2dbd4e84ab571aea39375c5f/0/live.m3u8";
    //var deliver = "http://172.16.0.31:6713/mag/hls/"+syscode+"/0/live.m3u8";
    //var deliver = "http://172.16.0.218/hls/EVMTV_172.16.1.130-554-av_stream/EVMTV_172.16.1.130-554-av_stream-.m3u8";
    //var deliver="http://172.16.0.218/hls/EVMTV_172.16.0.220-554-ch11-main-av_stream/EVMTV_172.16.0.220-554-ch11-main-av_stream-.m3u8";
    var deliver=src;
    if(deliver.length>10){
        if(deliver.substring(0,7)!='http://'&&deliver.substring(0,8)!='https://') deliver="http://"+deliver;
    }else{
        deliver="http://"+deliver;
    }
    var nativePlayerInstanceId = mp.getNativePlayerInstanceId();
    mp.setVideoDisplayMode(0);		//设置播放器播放窗口模式，按下面的设置区域位置来播放视频
    mp.setVideoDisplayArea(697,213,360,268); //这里设置播放区域	left, top, width, height
    //	mp.setvideodisplayarea(0,0,1280,720);
    mp.setVolume(16);		//设置默认音量为16
    //设置播放的内容
...
$(function () {
    ...
    //console.log(number, PartyBranch, StudyContent, status);
    //解析值
    function getParam(paramName) {
        paramValue = "";
        isFound = false;
    ...
    if (oWndInfo != null) {
        ...
        setInterval(function () {
            ...
            if (0 == iRet) {
                ...
                var PartyBranch = getParam('PartyBranch');
                var StudyContent = getParam('StudyContent');
                //console.log(PartyBranch, StudyContent);
                //解析值 
                function getParam(paramName) {
                    paramValue = "";
                    isFound = false;
                ...
                $.ajax({
                    url: svc_sys + "/SavePicture?",
                    type: "GET",
                    data: {
                        ImageURL: szPicName + '.jpeg',
