...
public class BrandUpload : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        try
        {
            ...
            string title, description, districtID;
            ...
            //记录随手拍数据
            var returnData = d.addBrand( title,  description,  districtID);
                ...
                if (fileLen > 0)
                {
                    if (!string.IsNullOrEmpty(context.Request.Files[0].FileName))
                    {
                        for (var i = 0; i < fileLen; i++)
                        {
                            ...
                            var returnData2 = d.AddBrandPicture(returnData.data, Url);
