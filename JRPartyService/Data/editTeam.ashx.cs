...
public class editTeam : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        try
        {
            ...
            string id, title, description, participation, districtID;
            ...
            //记录随手拍数据
            var returnData = d.editTeam(id,title,  description,  participation,  districtID);
                ...
                if (fileLen > 0)
                {
                    if (!string.IsNullOrEmpty(context.Request.Files[0].FileName))
                    {
                        for (var i = 0; i < fileLen; i++)
                        {
                            ...
                            var returnData2 = d.AddTeamPicture(id, Url);
