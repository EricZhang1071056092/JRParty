...
public class editVolunteerActivity : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        try
        {
            ...
            string id, description, districtID;
            id = context.Request.Params["id"];
            description = context.Request.Params["description"];
            districtID = context.Request.Params["districtID"];
            //记录随手拍数据
            var returnData = d.editVolunteerActivity(id, description, districtID);
                ...
                if (fileLen > 0)
                {
                    if (!string.IsNullOrEmpty(context.Request.Files[0].FileName))
                    {
                        for (var i = 0; i < fileLen; i++)
                        {
                            ...
                            var returnData2 = d.AddVolunteerActivityPicture(id, Url);
