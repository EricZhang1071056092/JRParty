...
public class editVolunteer : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        try
        {
            ...
            //记录随手拍数据
            var returnData = d.editVolunteer(id, name, IDCard, sex, nation, birthDay, JionTime, workTime, duty, education, TrainingTitle, type, phone, financialType, districtID);
                ...
                if (fileLen > 0)
                {
                    if (!string.IsNullOrEmpty(context.Request.Files[0].FileName))
                    {
                        for (var i = 0; i < fileLen; i++)
                        {
                            ...
                            var returnData2 = d.AddVolunteerPicture(id, Url);
